// ── global template: split-header ─────────────────────────────────────────────
// Name + title left-aligned, contacts right-aligned. Default section styles.
// ─────────────────────────────────────────────────────────────────────────────
...
#let contact-items = data.contact.map(c =>
  if c.url != "" { link(c.url)[#c.label] } else { c.label }
)
#let loc-parts = (data.location.city, data.location.country).filter(s => s != "")
#if loc-parts.len() > 0 {
  contact-items = contact-items + (text(size: 9.5pt, fill: luma(100))[#loc-parts.join(", ")],)
}
...
#grid(
  ...
  align(right + horizon)[
    #text(size: 9.5pt, fill: luma(80))[#contact-items.join(" • ")]
  ]
)
